        ...
        <TableCompo id="tableContainer"> 
            ...
            <template v-slot:body>
                <tr v-for="data in tableData" :key="data.id" class="border dark:bg-black100 bg-secondary10">
                    ...
                    <table-data-comp :data="data.persons"></table-data-comp>
                    <table-data-comp :data="data.duration"></table-data-comp>
                    <td class="px-3 sm:px-6 py-3">
                            ...
                            <div class="relative">
                                ...
                                <portal :to="data.id">
                                    <div :id="data.id" class="edit-drop z-30 fixed space-y-3 rounded shadow-lg p-4 w-max bg-white dark:bg-black100 text-base py-2">
                                        ...
                                        <router-link :to="{name:'package-update', params:{id:data.id} }" :data="data" class="flex items-center space-x-3">
...
import { PackageListRequest } from '../../../types/package';
...
let listFilterData : PackageListRequest={
        perPage:10,
        page:1,
        enabled:1,
        withLocations:0
...
export default defineComponent({
    ...
    setup(){
       const { actionModalID, actionButton, positionEditDrop, toggleActionDropdown } = actionDropdown();
        ...
        const setTableData = ()=>{
            new PackageService().listOfPackage(state.filterForm).then((response:any)=>{
