<template>
    <div v-show="total>10" class="mt-6">
        <div class="flex flex-wrap items-center -mx-4">
            <div class="md:w-1/2 w-full px-4">
                <p class="md:text-left text-center">{{ $t('message.showing') }} {{ perPage }} {{ $t('message.entries-of-total') }} {{ total }}</p>
            </div>
            <div class="md:w-1/2 w-full px-4 mt-5 md:mt-0 justify-end">
                <ul class="flex items-center md:justify-end justify-center space-x-4">
                    <li @click="$emit('prev')" class="flex items-center pr-2" :class="prevCursor !== '' ? 'cursor-pointer group':'cursor-not-allowed'">
                        ...
                        <span class="pl-2 group-hover:text-primary">{{ $t('message.prev') }}</span>
                    </li>
                    <li @click="$emit('next')" class="flex items-center pl-2" :class="nextCursor !=='' ? 'cursor-pointer group':'cursor-not-allowed'">
                        <span class="pr-2 group-hover:text-primary">{{ $t('message.next') }}</span>
